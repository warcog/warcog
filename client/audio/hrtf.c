...
float hrtf_calcfadetime(float oldGain, float newGain, const float *olddir, const float *newdir)
{
    ...
    if(gainChange > 0.0001f || newGain > 0.0001f)
    {
        ...
        {
            float dotp = newdir[0] * olddir[0] + newdir[1] * olddir[1] + newdir[2] * olddir[2];
